...
// ---------- Nav: ocultar al bajar, mostrar al subir (estilo YouTube) ----------
const navBar = document.querySelector('.nav');
let lastScrollY = window.scrollY;
let navHeight = navBar.offsetHeight; // se cachea; se recalcula solo en resize, no en cada scroll
const SCROLL_HYSTERESIS = 6; // px mínimos de movimiento para reaccionar (evita parpadeos por micro-scroll)
...
function updateNavVisibility() {
  ...
  if (currentY <= navHeight) {
    navBar.classList.remove('nav--hidden'); // siempre visible cerca del tope
  } else if (delta > SCROLL_HYSTERESIS) {
    navBar.classList.add('nav--hidden'); // bajando
  } else if (delta < -SCROLL_HYSTERESIS) {
    navBar.classList.remove('nav--hidden'); // subiendo
  }
  lastScrollY = currentY;
}

// Un solo listener de scroll, sincronizado con requestAnimationFrame para
// evitar recalcular estilos más veces de las que el navegador puede pintar.
...
// Un solo debounce de resize para todo lo relacionado al nav (altura + variable CSS)
let navResizeTimer = null;
window.addEventListener('resize', () => {
  clearTimeout(navResizeTimer);
  navResizeTimer = setTimeout(setNavHeightVar, 150);
});

// ---------- Grilla estilo Pinterest (masonry real, orden fila por fila) ----------
const grid = document.getElementById('grid-proyectos');
const allCards = grid ? Array.from(grid.querySelectorAll('.card')) : [];
let masonryRAF = null;
...
function layoutMasonry() {
  ...
  // Crear columnas nuevas y mover ahí las tarjetas visibles
  const columns = [];
  ...
  allCards.forEach(card => {
    ...
    // Las tarjetas ocultas (display:none) no suman altura real
    const isHidden = card.classList.contains('is-hidden');
    heights[shortest] += isHidden ? 0 : (card.offsetHeight || 260);
  });
...
// Reacomodar a medida que cada imagen carga (las alturas cambian)
allCards.forEach(card => {
...
// Reacomodar al cambiar tamaño de ventana (cambia cantidad de columnas)
let resizeTimer = null;
window.addEventListener('resize', () => {
  clearTimeout(resizeTimer);
  resizeTimer = setTimeout(scheduleMasonry, 150);
});
...
// Link único a la carpeta con los proyectos completos (videos, mejor calidad, etc.)
// Se define una sola vez aquí: para cambiarlo, solo hay que editar esta línea.
const FULL_PROJECT_LINK = 'https://drive.google.com/drive/folders/1xDUrlDfhLggF1MHfmTQFwpKfj4TyKNNZ?usp=sharing';
...
// Obtiene el texto largo de una card para el modal. Se busca primero en un
// <template class="card-full-caption"> dentro de la card (forma recomendada:
// se escribe el texto tal cual, sin pelear con comillas ni saltos de línea
// dentro de un atributo HTML). Si no existe, cae de vuelta a data-caption
// (por compatibilidad) y, en último caso, al texto corto visible en el grid.
function getCardCaption(card) {
  const template = card.querySelector('.card-full-caption');
  if (template) {
    return template.content.textContent.trim().replace(/\s+/g, ' ');
  }
  if (card.dataset.caption) return card.dataset.caption;
  const shortCaption = card.querySelector('.card-caption');
  return shortCaption ? shortCaption.textContent : card.querySelector('img').alt;
}
...
function openModal(card) {
  ...
  modalCaption.textContent = getCardCaption(card);

  // Aviso de "proyecto completo": se muestra solo si la card lo marca con
  // data-full-version="true" (audiovisuales y otros proyectos más extensos).
  if (card.dataset.fullVersion === 'true') {
    modalFullLink.href = card.dataset.fullVersionLink || FULL_PROJECT_LINK;
